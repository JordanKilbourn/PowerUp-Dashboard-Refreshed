...
(function (PowerUp) {
  ...
  const normCategory = v => (/^ci|improve/i.test(v||'')) ? 'CI' :
    (/^quality/i.test(v||'')) ? 'Quality' :
    (/^safety/i.test(v||'')) ? 'Safety' :
    (/^training/i.test(v||'')) ? 'Training' : 'Other';
  const catVar = cat => ({
    CI:'var(--sq-ci)', Quality:'var(--sq-quality)', Safety:'var(--sq-safety)',
    Training:'var(--sq-training)', Other:'var(--sq-other)'
  }[cat] || 'var(--accent)');

  const parseMemberTokens = text => String(text || '').split(/[,;\n]+/).map(s => s.trim()).filter(Boolean);
  ...
  async function load() {
    ...
    emRows.forEach(r=>{
      ...
      const name=pick(r,EMP_COL.name,'').trim();
      ...
      if(SHEETS.SQUAD_MEMBERS){
        ...
        smRows.forEach(r=>{
          ...
          if(role==='leader'){
            ...
            arr.push({id:eid,name:enm}); LEADERS_BY_SQUAD.set(sid,arr);
          }
        });
      }
    } catch(_){}
    ...
    ALL=rows.map(r=>{
      ...
      return {
        ...
        active:pick(r,SQUAD_COL.active,''), created:pick(r,SQUAD_COL.created,''),
  ...
  function initManageSquadsFeature(manageBtn) {
    ...
    function showOverlay(text="Loading..."){
      ...
      if(!overlay){
        ...
        overlay.innerHTML=`<div class="manage-overlay-spinner"><div class="spinner"></div><div>${text}</div></div>`;
...
table.innerHTML = `<thead><tr>
  ...
  <tr data-id="${r["Squad ID"] || ""}" data-rowid="${r.id || ""}">
    ...
    <td class="actions-cell">
        ...
        btn.addEventListener("click",async e=>{
          ...
          try {
            await PowerUp.api.updateRowById("SQUADS",rowId,{
              "Squad Name":name,"Category":category,"Leader":leader,
              "Active":active,"Objective":objective,"Created By":createdBy,
            });
            tr.querySelectorAll("[data-original]").forEach(c=>c.dataset.original=c.textContent.trim());
            ...
            PowerUp.ui?.toast?.("Error updating squad.","error");
          } finally { hideOverlay(); setTimeout(()=>tr.style.background="",1200); }
        });
      });
      ...
      manageView.querySelectorAll(".cancel-btn").forEach(btn=>{
        btn.addEventListener("click",e=>{
          const tr=e.target.closest("tr");
          tr.querySelectorAll("[data-original]").forEach(c=>c.textContent=c.dataset.original);
          const chk=tr.querySelector("input[type=checkbox]");
          if(chk) chk.checked=chk.dataset.original==="true";
          tr.style.transition="background-color .4s"; tr.style.backgroundColor="rgba(255,255,0,0.1)";
    ...
    style.textContent=`#manageOverlay{position:fixed;inset:0;background:rgba(0,0,0,.6);display:none;align-items:center;justify-content:center;z-index:3000;}
      ...
      .manage-table th{background:#0f1a1a;color:#9ffbe6;font-weight:600;text-transform:uppercase;font-size:12px;position:sticky;top:0;z-index:10;}
      .manage-table td[contenteditable]{background:#101f1f;}
      .actions-cell{display:flex;gap:8px;justify-content:center;}
      .actions-cell .btn{min-width:70px;padding:4px 10px;border-radius:6px;border:1px solid var(--accent,#00f08e);background:#0f1a1a;color:#d9e6e6;cursor:pointer;}
      .actions-cell .btn:hover{background:#152525;}`;
