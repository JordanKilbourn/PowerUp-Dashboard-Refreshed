// =============================================
// PowerUp: Squads Page – Reconciled Build
// =============================================
(function (PowerUp) {
    ...
    table.innerHTML = `
      ...
      <tbody>
        ${squads.map(r => {
          ...
          return `
          <tr data-rowid="${sid}" data-original='${JSON.stringify(original)}'>
            <td>${sid}</td>
            ...
            <td>
              <select multiple class="leader-select">
                ${allEmps.map(emp => `
                  <option value="${emp.id}" ${leaders.includes(emp.name) ? 'selected' : ''}>${emp.name}</option>`).join('')}
              </select>
            </td>
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (btnAdd) {
      btnAdd.addEventListener('click', () => {
        const f = PowerUp.squadAddForm;
        if (!f) return;
        if (typeof f.open === 'function') f.open();
        else if (typeof f.openModal === 'function') f.openModal();
      });
    }

    if (btnManage) {
      btnManage.addEventListener('click', async () => {
        const isView = btnManage.dataset.mode === 'manage';
        if (!isView) {
  ...
  style.textContent = `
    ...
    .squad-card {
      ...
      border-left: 6px solid var(--sq-other);
      border-radius: 10px;
      padding: 1rem;
    ...
    .cat-ci { border-left-color: var(--sq-ci); }
    .cat-quality { border-left-color: var(--sq-quality); }
    .cat-safety { border-left-color: var(--sq-safety); }
    .cat-training { border-left-color: var(--sq-training); }
    .cat-other { border-left-color: var(--sq-other); }
    ...
    /* === Row Feedback === */
    .flash { background: rgba(255,255,0,0.1) !important; }
    .saving { opacity: 0.6; }
    .saved { background: rgba(51,255,153,0.08) !important; }
  `;
